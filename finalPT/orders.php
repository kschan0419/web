...
<head>
  ...
  <title>My Bag Ordering System : Orders</title>
  <!-- Bootstrap -->
...
<div class="container-fluid">
  <div class="row">
    ...
    <form action="orders.php" method="post" class="form-horizontal">
      ...
      <input name="oid" type="text" class="form-control" id="orderid" readonly value="<?php if(isset($_GET['edit'])) echo $editrow['fld_order_num']; ?>" required> 
      ...
      <input name="orderdate" type="text" class="form-control" id="orderdate" readonly value="<?php if(isset($_GET['edit'])) echo $editrow['fld_order_date']; ?>" required> 
      </div>
        </div>
      <div class="form-group">
      ...
      try {
        ...
        $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
          $stmt = $conn->prepare("SELECT * FROM tbl_staffs_a160841_pt2");
        $stmt->execute();
      ...
      try {
        $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
        $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
          $stmt = $conn->prepare("SELECT * FROM tbl_customers_a160841_pt2");
        $stmt->execute();
        $result = $stmt->fetchAll();
      }
      catch(PDOException $e){
    ...
    <div class="col-xs-12 col-sm-10 col-sm-offset-1 col-md-8 col-md-offset-2">
      ...
      try {
        $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
        $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $sql = "SELECT * FROM tbl_orders_a160841, tbl_staffs_a160841_pt2, tbl_customers_a160841_pt2 WHERE ";
        $sql = $sql."tbl_orders_a160841.fld_staff_id = tbl_staffs_a160841_pt2.fld_staff_id and ";
        $sql = $sql."tbl_orders_a160841.fld_customer_id = tbl_customers_a160841_pt2.fld_customer_id LIMIT $start_from, $per_page";
    ...
    <div class="col-xs-12 col-sm-10 col-sm-offset-1 col-md-8 col-md-offset-2">
      <nav>
          ...
          try {
            $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
            $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stmt = $conn->prepare("SELECT * FROM tbl_orders_a160841");
            $stmt->execute();
            $result = $stmt->fetchAll();
